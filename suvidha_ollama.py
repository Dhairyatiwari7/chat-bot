...
def ollama_embed_function(texts):
    """Wrapper function for Ollama embeddings to work with FAISS"""
    try:
        embeddings = []
        for text in texts:
            embedding = ollama_embeddings.embed_text(text)
            if embedding:
        ...
        print(f"Error in ollama_embed_function: {e}")
        # Return zero vectors as fallback
        return [[0.0] * 768 for _ in texts]

def get_vector_store(text_chunks):
    """Create vector store from text chunks using Ollama embeddings"""
    try:
        # Use our custom embedding function
        vector_store = FAISS.from_texts(text_chunks, embedding=ollama_embed_function)
...
def get_vector_store_image(text_chunks):
    """Create vector store for image text using Ollama embeddings"""
    try:
        # Use our custom embedding function
        vector_store = FAISS.from_texts(text_chunks, embedding=ollama_embed_function)
...
def process_document_query(user_question):
    """Process document queries using Ollama"""
    try:
        # Load FAISS vector store with our embedding function
        vector_store = FAISS.load_local("faiss_index", ollama_embed_function, allow_dangerous_deserialization=True)
...
def process_image_query(user_question):
    """Process image queries using Ollama"""
    try:
        # Load image FAISS vector store with our embedding function
        vector_store = FAISS.load_local("faiss_index_image", ollama_embed_function, allow_dangerous_deserialization=True)
